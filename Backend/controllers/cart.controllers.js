...
const addToCart = asyncHandler(async (req, res,next) => {
    ...
    if (!productDetails) {
      return next(new ApiError(400, "Product details are required"));
    }

    const { productId, quantity, color, colorCode, size } = productDetails;
    const { id: userId } = user;

    if (!productId || !quantity || !color || !colorCode || !size) {
      return next(new ApiError(400, "Product id, quantity, color, color code, and size are required"));
    }
    ...
    const selectedColor = selectedVariation?.colors.find((item) => item.color === color && item.colorCode === colorCode);

    if (!selectedVariation || !selectedColor) {
      return next(new ApiError(400, "Selected product variation is not available"));
    }

    if (selectedColor.stock < quantity) {
      return next(new ApiError(400, `Insufficient stock: ${selectedColor.stock} left`));
    }
...
const removeCartItem = asyncHandler(async(req,res,next)=>{
    ...
    if (!productId || !color || !size) {
      return next(new ApiError(400, "Product id, color, and size are required"));
    }
